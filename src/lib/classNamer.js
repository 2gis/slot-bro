 ...
 */
export function moduleClass(moduleName) {
    return moduleName;
}
 ...
 */
export function elementClass(moduleName, elementName) {
 ...
 */
export function modificatorClass(name, value) {
 ...
 */
export function parseMods(classList) {
 ...
 */
export function stringifyMods(mods) {
    if (!_.isObject(mods)) {
        throw new TypeError('Mods must be an Object');
    }

    var classes = [];

    _.each(mods, function(val, key) {
        var modClass = modificatorClass(key, val);
